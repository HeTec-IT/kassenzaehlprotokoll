...
function recipientsFor(cfg, shopId){
  const e = cfg && cfg.empfaenger && cfg.empfaenger[shopId];
  if (Array.isArray(e)) return { to:e.length ? e : [CONFIG.MAIL_TO], cc:[] };
  return { to:(e && e.to && e.to.length) ? e.to : [CONFIG.MAIL_TO], cc:(e && e.cc) || [] };
}
...
/* ---------- Mail ---------- */
const eurDe = v => Number(v).toFixed(2).replace('.', ',') + ' €';
function buildMail(p, cfg){
  const shop = shopById(p.shop);
  const ok = isOk(p.diff);
  const k = p.kasse, d = dDay(p.t);
  const diffTxt = `${eurS(p.diff)}${ok ? '' : p.diff < 0 ? ' (fehlt)' : ' (Überschuss)'}`;
  const subject = ok
    ? `Kassenprotokoll ${shop.name} (${k}) ${p.schicht} ${d}`
    : `ACHTUNG: ${shop.name} ${k} stimmt nicht!`;
  const L = [];
  if (!ok) {
    L.push('======================================================',
           `❗❗❗ ACHTUNG: Der Kassenbestand stimmt nicht überein (${k}) ❗❗❗`,
           '======================================================', '',
           `Gesendet von: ${shop.name}`,
           `Ist-Betrag: ${eur(p.ist)}`, `Soll-Betrag: ${eur(p.soll)}`, `Differenz: ${diffTxt}`);
    if (p.bemerkung) L.push(`Bemerkung: ${p.bemerkung}`);
    L.push(`Unterschrift: ${p.name}`, '', '------------------------------------------------------');
  }
  const sec = { n:`--- Kassenbestand (Bargeld, ${k}) ---`, r:'--- Münzrollen ---', v:'--- Voucher ---' };
  const rows = key => GROUPS.find(g => g.key === key).items.map((it,i) => ({ it, n:p.counts[key][i] })).filter(x => x.n > 0);
  L.push(sec.n);
  [...rows('n'), ...rows('c')].forEach(({it,n}) => L.push(`${eurDe(it.unit)} x ${n} = ${eurDe(it.unit*n)}`));
  const rr = rows('r');
  if (rr.length) { L.push('', sec.r); rr.forEach(({it,n}) => L.push(`${it.label} Rolle (${it.hint.match(/\d+ St\./)[0]}) x ${n} = ${eurDe(it.unit*n)}`)); }
  const vv = rows('v');
  if (vv.length) { L.push('', sec.v); vv.forEach(({it,n}) => L.push(`${eurDe(it.unit)} Voucher x ${n} = ${eurDe(it.unit*n)}`)); }
  L.push('', '--- Zusammenfassung ---', `Shop: ${shop.name}`, `Kasse: ${k}`, `Schicht: ${p.schicht} (${dFull(p.t)})`,
         `Soll: ${eur(p.soll)}`, `Ist: ${eur(p.ist)}`);
  ...
  L.push('', `Unterschrift: ${p.name}`);
  const body = L.join('\n');
  const r = recipientsFor(cfg, p.shop);
  const cc = r.cc.length ? `cc=${encodeURIComponent(r.cc.join(','))}&` : '';
  return { subject, body, to:r.to, cc:r.cc, href:`mailto:${r.to.join(',')}?${cc}subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}` };
}
  ...
  return (
    <div>
      ...
      <details style={{marginTop:10}}><summary className="small muted" style={{cursor:'pointer'}}>Mail-Vorschau</summary>
        <pre className="small" style={{whiteSpace:'pre-wrap',background:'var(--soft)',padding:10,borderRadius:8,marginTop:8}}>{'An: ' + m.to.join(', ') + (m.cc.length ? '\nCC: ' + m.cc.join(', ') : '') + '\nBetreff: ' + m.subject + '\n\n' + m.body}</pre>
      </details>
    </div>
  );
}
...
function SettingsTab({ cfg, toast }){
  const init = () => Object.fromEntries(SHOPS.map(s => { const r = recipientsFor(cfg, s.id); return [s.id, { to:r.to.join('\n'), cc:r.cc.join('\n') }]; }));
  const [emp, setEmp] = useState(init);
  const [busy, setBusy] = useState(false);
  const setF = (id, f, v) => setEmp(x => ({ ...x, [id]:{ ...x[id], [f]:v } }));
  async function save(){
    const empfaenger = {};
    for (const s of SHOPS) {
      const to = parseMails(emp[s.id].to), cc = parseMails(emp[s.id].cc).filter(x => !to.includes(x));
      if (!to.length) return toast(`${s.name}: bei „An“ mindestens eine E-Mail eintragen.`);
      empfaenger[s.id] = { to, cc };
    }
    setBusy(true);
    try { await cfgRef.set({ empfaenger }, { merge:true }); toast('Gespeichert'); }
  ...
  return (
    <div className="card">
      ...
      <p className="muted small">Eine Adresse pro Zeile. Gilt für jedes Protokoll des Shops – bei Differenz mit Betreff „ACHTUNG: … stimmt nicht!“.</p>
      <div className="sgrid" style={{marginBottom:14}}>
        {SHOPS.map(s => (
          <div key={s.id} style={{border:'1px solid var(--line)',borderRadius:'var(--r)',padding:12}}>
            <h3 style={{marginBottom:10}}><span className={'dot ' + s.brand} style={{marginRight:6}}></span>{s.name}</h3>
            <label className="field"><span>An ({parseMails(emp[s.id].to).length})</span>
              <textarea className="inp" style={{minHeight:60}} value={emp[s.id].to} onChange={e => setF(s.id,'to',e.target.value)} /></label>
            <label className="field" style={{margin:0}}><span>CC ({parseMails(emp[s.id].cc).length})</span>
              <textarea className="inp" style={{minHeight:60}} value={emp[s.id].cc} onChange={e => setF(s.id,'cc',e.target.value)} placeholder="optional" /></label>
          </div>
        ))}
      </div>
