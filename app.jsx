...
function useConfig(){
  ...
  useEffect(() => cfgRef.onSnapshot(d => setC(d.exists ? d.data() : {}), () => setC({})), []);
  return c;
}
const recipientsFor = (cfg, shopId) => { const l = cfg && cfg.empfaenger && cfg.empfaenger[shopId]; return (l && l.length) ? l : [CONFIG.MAIL_TO]; };
const parseMails = t => [...new Set(String(t || '').split(/[\s,;]+/).map(x => x.trim().toLowerCase()).filter(x => /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(x)))];

/* Warnmail automatisch über EmailJS (REST, ohne Bibliothek) */
async function sendMailJS(ej, to, subject, message, shopName){
  if (!ej || !ej.serviceId || !ej.templateId || !ej.publicKey) return 'nicht eingerichtet';
  const ctrl = new AbortController(); const tm = setTimeout(() => ctrl.abort(), 10000);
  try {
    const r = await fetch('https://api.emailjs.com/api/v1.0/email/send', {
      method:'POST', headers:{ 'Content-Type':'application/json' }, signal:ctrl.signal,
      body: JSON.stringify({ service_id:ej.serviceId, template_id:ej.templateId, user_id:ej.publicKey,
        template_params:{ to_email:to.join(','), subject, message, shop:shopName } })
    });
    clearTimeout(tm);
    return r.ok ? 'gesendet' : ('Fehler ' + r.status + ': ' + (await r.text()).slice(0,100));
  } catch(e){ clearTimeout(tm); return navigator.onLine ? 'Fehler: keine Antwort' : 'offline'; }
}
function sendAlarm(p, cfg){
  const m = buildMail(p, cfg);
  return sendMailJS(cfg && cfg.emailjs, recipientsFor(cfg, p.shop), m.subject, m.body, shopById(p.shop).name);
}
...
function buildMail(p, cfg){
  ...
  const to = recipientsFor(cfg, p.shop);
  return { subject, body, to, href:`mailto:${to.join(',')}?${cc}subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}` };
}
...
function CountForm({ shop, kasse, mode, prefill, takeover, cfg, onSave, onCancel }){
  ...
  async function submit(){
    ...
    setBusy(true);
    if (!visit && !isOk(p.diff)) p.alarm = await sendAlarm(p, cfg);   // separate Warnmail an alle Empfänger des Shops
  ...
  return (
    <div className="layout">
      ...
      <aside className={'tally ' + state} aria-live="polite">
        ...
        <button className="btn" disabled={busy} onClick={submit}>{busy ? 'Wird gespeichert …' : visit ? 'Zählung speichern' : 'Protokoll speichern & senden'}</button>
    ...
    <li className={(ok ? '' : 'bad') + (isOrigin ? ' start' : '')}>
      <details onToggle={e => setOpen(e.currentTarget.open)}>
        <summary>
          ...
          {(tag || p.source === 'besuch' || p.fotos > 0 || p._pending || p.alarm) && <div className="tags">
            ...
            {p.alarm === 'gesendet' && <span className="tag">Warnmail gesendet</span>}
            {p.alarm && p.alarm !== 'gesendet' && <span className="tag amber">Warnmail nicht gesendet</span>}
          </div>}
...
function MailBox({ p, cfg }){
  const m = buildMail(p, cfg); const [c, setC] = useState(false);
  return (
    <div>
      <div className="btns">
      ...
      <details style={{marginTop:10}}><summary className="small muted" style={{cursor:'pointer'}}>Mail-Vorschau</summary>
        <pre className="small" style={{whiteSpace:'pre-wrap',background:'var(--soft)',padding:10,borderRadius:8,marginTop:8}}>{'An: ' + m.to.join(', ') + '\nBetreff: ' + m.subject + '\n\n' + m.body}</pre>
      </details>
    </div>
  );
}
        ...
        {tab === 'count' && (sent && sent.kasse === kasse ? (
          <div>
            ...
            <div className="card">
              ...
              {!isOk(sent.diff) && (sent.alarm === 'gesendet'
                ? <p className="note" style={{background:'var(--okbg)',color:'var(--ok)'}}>Warnmail „Kasse passt nicht“ wurde automatisch an {recipientsFor(cfg, shop.id).join(', ')} gesendet.</p>
                : <p className="note">Warnmail konnte nicht automatisch gesendet werden ({sent.alarm}). Bitte die E-Mail unten senden.</p>)}
              <MailBox p={sent} cfg={cfg} />
          ...
          <>
            ...
            <CountForm key={kasse + formKey} shop={shop} kasse={kasse} mode="shop" cfg={cfg} onSave={p => { setSent(p); window.scrollTo(0,0); }} />
          </>
        ))}
...
function SettingsTab({ cfg, toast }){
  const [emp, setEmp] = useState(() => Object.fromEntries(SHOPS.map(s => [s.id, recipientsFor(cfg, s.id).join('\n')])));
  const [ej, setEj] = useState({ serviceId:'', templateId:'', publicKey:'', ...(cfg.emailjs || {}) });
  const [busy, setBusy] = useState(false);
  const setE = (k,v) => setEj(x => ({ ...x, [k]:v.trim() }));
  async function save(){
    const empfaenger = {};
    for (const s of SHOPS) {
      const l = parseMails(emp[s.id]);
      if (!l.length) return toast(`${s.name}: mindestens eine gültige E-Mail eintragen.`);
      empfaenger[s.id] = l;
    }
    setBusy(true);
    try { await cfgRef.set({ empfaenger, emailjs:ej }, { merge:true }); setEmp(Object.fromEntries(SHOPS.map(s => [s.id, empfaenger[s.id].join('\n')]))); toast('Gespeichert'); }
    catch(e){ toast('Speichern fehlgeschlagen: ' + (e.code === 'permission-denied' ? 'Firestore-Regeln prüfen' : e.message)); }
    setBusy(false);
  }
  async function test(){
    setBusy(true);
    const r = await sendMailJS(ej, [CONFIG.MAIL_TO], 'TEST: Warnmail Kassenzählprotokoll', 'Testnachricht – die automatische Warnmail ist eingerichtet.', 'Test');
    toast(r === 'gesendet' ? `Testmail an ${CONFIG.MAIL_TO} gesendet` : 'Test fehlgeschlagen – ' + r);
    setBusy(false);
  }
  return (
    <>
      <div className="card">
        <h2>E-Mail-Empfänger pro Shop</h2>
        <p className="muted small">Eine Adresse pro Zeile. Diese Personen bekommen das Protokoll – und bei Differenz zusätzlich automatisch eine separate Warnmail „Kasse passt nicht“.</p>
        <div className="sgrid">
          {SHOPS.map(s => (
            <label className="field" key={s.id} style={{margin:0}}>
              <span><span className={'dot ' + s.brand} style={{marginRight:6}}></span>{s.name} ({parseMails(emp[s.id]).length})</span>
              <textarea className="inp" style={{minHeight:96}} value={emp[s.id]} onChange={e => setEmp(x => ({ ...x, [s.id]:e.target.value }))} />
            </label>
          ))}
        </div>
      </div>
      <div className="card">
        <h2>Automatische Warnmail (EmailJS)</h2>
        <p className="muted small">Einmalig auf emailjs.com: Konto anlegen → E-Mail-Dienst verbinden (Outlook/Microsoft 365) → Vorlage mit An = {'{{to_email}}'}, Betreff = {'{{subject}}'}, Inhalt = {'{{message}}'}. Kostenlos bis 200 Mails/Monat.</p>
        <div className="grid2">
          <label className="field"><span>Service ID</span><input className="inp" value={ej.serviceId} onChange={e => setE('serviceId', e.target.value)} placeholder="service_…" /></label>
          <label className="field"><span>Template ID</span><input className="inp" value={ej.templateId} onChange={e => setE('templateId', e.target.value)} placeholder="template_…" /></label>
        </div>
        <label className="field"><span>Public Key</span><input className="inp" value={ej.publicKey} onChange={e => setE('publicKey', e.target.value)} /></label>
        <div className="btns">
          <button className="btn" disabled={busy} onClick={save}>Speichern</button>
          <button className="btn ghost" disabled={busy} onClick={test}>Testmail an mich</button>
        </div>
      </div>
    </>
  );
    ...
    <>
      ...
      <header className="top">
        ...
        {!shop && <nav className="tabs">
          {[['shops','Shops'],['settings','Empfänger'],['links','Shop-Links']].map(([k,l]) =>
      ...
      <main className="wrap">
        ...
        {loading && !list.length ? <div className="center">Lade Daten …</div> :
          ...
          tab === 'settings' ? (cfg ? <SettingsTab cfg={cfg} toast={setMsg} /> : <div className="center">Lade …</div>) :
